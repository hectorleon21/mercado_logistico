...
from .models import Provider, Service, Event, News, Rubro, NewsletterSubscription, Newsletter, Course, Article, Requirement, Proposal
...
class ProposalAdmin(AnuncioAdminBase):
    list_display = ('title', 'company', 'provider', 'phone', 'email', 'valid_until', 'is_active', 'created_at')
    list_filter = ('is_active', 'valid_until')
    search_fields = ('title', 'company', 'description', 'email')
    date_hierarchy = 'created_at'
    ...
    fieldsets = (
        ('Información de la Empresa', {
            'fields': ('provider', 'company', 'title', 'description')
        }),
        ('Información de Contacto', {
            'fields': ('phone', 'email')
        }),
    ...
    class Media:
        js = ('admin/js/proposal_provider_autofill.js',)

# Ahora registramos con admin.site para poder personalizar
admin.site.register(Requirement, RequirementAdmin)
admin.site.register(Proposal, ProposalAdmin)

# Ahora manejaremos la agrupación modificando el AdminSite
from django.contrib.admin import AdminSite

# Esto debe ir al final del archivo admin.py
class CustomAdminSite(AdminSite):
    ...
    def get_app_list(self, request):
        """
        Personalizar el menú del administrador para agrupar modelos
        """
        app_list = super().get_app_list(request)
        ...
        if logistics_app:
            ...
            # Mover Requerimientos y Propuestas al nuevo grupo
            ...
            # Insertar el nuevo grupo después de logistics
            if anuncios_group['models']:
                logistics_index = app_list.index(logistics_app)
                app_list.insert(logistics_index + 1, anuncios_group)
        
        return app_list

# Registrar el nuevo AdminSite
admin_site = CustomAdminSite(name='custom_admin')

# Registrar modelos de autenticación en el admin_site personalizado
admin_site.register(User, UserAdmin)
admin_site.register(Group, GroupAdmin)

# Registrar todos los modelos con admin_site
